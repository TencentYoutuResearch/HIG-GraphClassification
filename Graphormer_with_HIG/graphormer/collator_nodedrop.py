...
class Batch():
    def __init__(self, idx, attn_bias, attn_edge_type, rel_pos, in_degree, out_degree, x, edge_input, y, edge_index):
...
def collator(items, max_node=512, multi_hop_max_dist=20, rel_pos_max=20):
    ...
    items = [(item.idx, item.attn_bias, item.attn_edge_type, item.rel_pos, item.in_degree,
              ...
              item.edge_index) for item in items]
    idxs, attn_biases, attn_edge_types, rel_poses, in_degrees, out_degrees, xs, edge_inputs, ys, edge_index = zip(
        *items)
