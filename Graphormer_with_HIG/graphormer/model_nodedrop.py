...
from data_new import get_dataset
...
def propagate(feature, A, order):
    ...
    a,b,c = feature.shape 
    #print("a,b,c: ", a,b,c)
    x = torch.reshape(feature, (a, b*c))
    y = feature
    #print("propagate A: ", A.shape)
    #print("x: ", x.shape)
    #print("type A: ", type(A))
    #print("type x: ", type(x))    
    #print(A)
    ...
    #A = A.long()
    #x = x.long()
    #A2 = tf.sparse_tensor_to_dense(A, default_value=0)
    A2 = A.to_dense()
    #print("shape A2: ", A2.shape)
    #print(A2)
    for i in range(order):
        x = torch.spmm(A2, x).detach_()
        y.add_(torch.reshape(x, (a,b,c)))
    #print("x.shape: ", x.shape)
    #print("y.shape: ", y.shape)

    return torch.unsqueeze(y.div_(order+1.0).detach_(), 0)
...
class Graphormer(pl.LightningModule):
    def __init__(
        ...
        if dataset_name == 'PCQM4M-LSC':
            ...
            self.downstream_out_proj_ft = nn.Linear(
    ...
    def forward(self, batched_data, perturb=None):
        ...
        order = 1
        N = n_node
        #print("n graph: ", n_graph)
        #print("x.shape: ", x.shape)
        for i in range(n_graph):
            #tmp = self.atom_encoder(x)
            #print("tmp shape: ", tmp.shape)
            new_fea = self.atom_encoder(x[i])
            #N = new_fea.size(0)
            #print("N: ", N)
            edge_index_fea = edge_index[i].cuda()
            #print("new_fea shape: ", new_fea.shape)
            #ori_fea, edge_index_fea = new_fea, edge_index[i].cuda()
            #print("x[i].shape: ", x[i].shape)
            #print("atom_encoder_x[i].shape: ", ori_fea.shape)
            #print("edge_index_fea.shape: ", edge_index_fea.shape)

            #print(x[i])
            #print(ori_fea)
            #print(edge_index_fea)
            ...
            if self.train_status:
                ...
                masks = torch.bernoulli(1. - drop_rates).unsqueeze(1).unsqueeze(1)
                #print("masks.shape: ", masks.shape)
                #print("new_fea.shape: ", new_fea.shape)
                new_fea = masks.cuda() * new_fea
                #print("after process new_fea: ", new_fea.shape)
            else:
                new_fea = new_fea*(1.-drop_rate)
            #ori_fea = new_fea
            adj = get_adj_matrix(edge_index_fea, N).cuda()#to(edge_index.device)
            #print("adj.shape: ", adj.shape)
            #print("adj type: ", type(adj))
            
 
            #adj = batched_data.adj[i]
            #order = 1
        ...
        #print("nodefeature shape:", node_feature.shape) 
        ...
        if self.dataset_name == 'PCQM4M-LSC':
            ...
            output = self.downstream_out_proj_ft(output[:, 0, :])  # virtual node
    ...
    def training_step(self, batched_data, batch_idx):
        if self.dataset_name == 'ogbg-molpcba':
            ...
            if not self.flag:
                ...
                ##y_gt = batched_data.y[:, 0].float().to(device)
                y_gt = batched_data.y[:, 0].float()
                def forward(perturb): return self(batched_data, perturb)
                model_forward = (self, forward)
                n_graph, n_node = batched_data.x.size()[:2]
    ...
    def validation_step(self, batched_data, batch_idx):
        if self.dataset_name in ['PCQM4M-LSC','PCQM4Mv2','ZINC']:
